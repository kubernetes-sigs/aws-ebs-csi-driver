...
func createMockObjects(mockCtrl *gomock.Controller, mountPath string) (*FakeMounter, *FakeCloud, *metadata.MockMetadataService, *d.MockKubernetesClient) {
	...
	fakeMetadataService.EXPECT().GetAvailabilityZone().Return(availabilityZone).AnyTimes()
	fakeMetadataService.EXPECT().GetOutpostArn().Return(*outpostArn).AnyTimes()
	fakeMetadataService.EXPECT().GetInstanceID().Return(instanceID).AnyTimes()
	fakeMetadataService.EXPECT().GetRegion().Return(region).AnyTimes()
