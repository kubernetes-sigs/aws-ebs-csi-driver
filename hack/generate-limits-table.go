...
const outputTemplate = `// Code generated by hack/generate-limits-table.go. DO NOT EDIT.
	...
	for paginator.HasMorePages() {
		...
		for _, instanceType := range page.InstanceTypes {
			...
			case instanceType.EbsInfo == nil:
				log.Printf("Skipping %s: EbsInfo is nil", instanceType.InstanceType)
				continue
			case instanceType.EbsInfo.MaximumEbsAttachments == nil:
				log.Printf("Skipping %s: MaximumEbsAttachments is nil", instanceType.InstanceType)
				continue
			case instanceType.EbsInfo.AttachmentLimitType != attachmentDedicated && instanceType.EbsInfo.AttachmentLimitType != attachmentShared:
				log.Printf("Skipping %s: AttachmentLimitType is %s (not dedicated or shared)", instanceType.InstanceType, instanceType.EbsInfo.AttachmentLimitType)
				continue
			}
...
func main() {
	...
	for _, region := range regions {
		...
		// Merge instances (later regions override earlier ones)
	...
	for instanceType, instance := range allInstances {
		if instance.MaxAttachments == 40 && instance.AttachmentType == attachmentShared {
			nonNitroInstances = append(nonNitroInstances, instanceType)
		} else if instance.MaxAttachments != 27 || instance.AttachmentType != attachmentShared {
