...
type templateData struct {
	...
	EbsCardCounts     map[string]int32
}
...
{{- range $instanceType, $cards := .EbsCardCounts}}
	"{{$instanceType}}": {{$cards}},
{{- end}}
}
`
...
func main() {
	...
	for _, region := range regions {
		...
		for k, v := range instances {
			if existing, exists := allInstances[k]; exists {
				...
				if (existing.MaxEbsCards == nil) != (v.MaxEbsCards == nil) || (existing.MaxEbsCards != nil && v.MaxEbsCards != nil && *existing.MaxEbsCards != *v.MaxEbsCards) {
					log.Fatalf("Instance type %s has different MaximumEbsCards in different regions: %+v vs %+v", k, existing.MaxEbsCards, v.MaxEbsCards)
				}
			}
			allInstances[k] = v
		}
	...
	ebsCardCounts := make(map[string]int32)
	...
	for instanceType, instance := range allInstances {
		...
		if instance.MaxEbsCards != nil && *instance.MaxEbsCards > 1 {
			ebsCardCounts[instanceType] = *instance.MaxEbsCards
		}
	}
