...
export ALL_OSVERSION_windows="ltsc2019 ltsc2022"
export ALL_ARCH_linux="amd64 arm64"
export ALL_ARCH_windows="amd64"
...
function build_and_push() {
  ...
  loudecho "Building and pushing test driver images to ${IMAGE_NAME}:${IMAGE_TAG}"
...
for suffix in ${imageSuffixes[@]}; do
  ...
  #Add all images to be deleted with imageTag at the beginning as we need it for the batch-delete-image call
  deleteImages[$i]="imageTag="${TAG}"-"${suffix}" "
  i+=1
done
...
loudecho "Deleting image indexes"
aws ecr batch-delete-image --registry-id "${AWS_ACCOUNT_ID}" --repository-name aws-ebs-csi-driver --image-ids imageTag=${TAG} imageTag=${TAG}-fips
loudecho "Deleting images"
aws ecr batch-delete-image --registry-id "${AWS_ACCOUNT_ID}" --repository-name aws-ebs-csi-driver --image-ids ${deleteImages[@]}
