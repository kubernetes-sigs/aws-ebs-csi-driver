	...
	for {
		...
		case t := <-b.taskChan:
			if _, exists := b.pendingTasks[t.task]; exists {
				klog.Infof("[batching-stats-collection] taskManager: duplicate task detected: %v", t.task)
			} else {
...
func (b *Batcher[InputType, ResultType]) execute(pendingTasks map[InputType][]chan BatchResult[ResultType]) {
	...
	klog.InfoS("[batching-stats-collection] execute: finished execution", "batchSize", len(batch))
}
