...
var (
	// metadataLabelerUpdateInterval is interval between attempts to update metadata by getting Node resource
	// Jitter between node service replicas to avoid spiking K8s API Server.
	//nolint:gosec // This use of rand is just for jitter.
	metadataLabelerUpdateInterval = 1*time.Hour + time.Duration(rand.Intn(10))*time.Minute
)
