...
func TestNewMetadataService(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.IMDSError == nil && !tc.imdsDisabled && (strings.Contains(tc.metadataSources, SourceIMDS)) {
			...
			cfg := MetadataServiceConfig{
				MetadataSources: strings.Split(tc.metadataSources, ","),
