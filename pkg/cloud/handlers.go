...
// of throttle errors vs server/unknown errors, to prevent flooding the logs with throttle errors
			...
			if err != nil {
				...
				if errors.As(err, &apiErr) {
					if _, isThrottleError := retry.DefaultThrottleErrorCodes[apiErr.ErrorCode()]; isThrottleError {
						...
						klog.V(6).ErrorS(apiErr, "Throttle error from AWS API")
					} else {
						klog.V(4).ErrorS(apiErr, "Error from AWS API")
					}
				} else {
					klog.V(4).ErrorS(err, "Unknown error attempting to contact AWS API")
				}
			}
			return output, metadata, err
		}), middleware.After)
