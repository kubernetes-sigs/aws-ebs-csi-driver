...
func (o *Options) AddFlags(f *flag.FlagSet) {
	...
	f.StringVar(&o.MetadataSources, "metadata-sources", "imds,kubernetes", "Dictates which sources are used to retrieve instance metadata. The driver will attempt to rely on each source in order until one succeeds. Valid options include 'imds' and 'kubernetes'.")
...
func (o *Options) Validate() error {
	...
	for _, source := range strings.Split(o.MetadataSources, ",") {
		if source != metadata.SourceIMDS && source != metadata.SourceK8s {
			return metadata.InvalidSourceErr(strings.Split(o.MetadataSources, ","), source)
		}
	}
